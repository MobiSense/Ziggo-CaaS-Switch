...
void setup_topo() {
    ...
    std::cout << "[INFO] current node is a " << type << " with id " << id << std::endl;
    if (id == -1) {
        std::cout << "[ERROR] Could not find node with mac address " << mac_addr << std::endl;
        exit(1);
    }
    ...
    for (auto &link: j["fwd"]) {
        ...
        if (src == id) {
            ...
            if (nodes.find(dst) == nodes.end()) {
                std::cout << "[ERROR] Could not find node with id " << dst << std::endl;
                exit(1);
            }
            std::string dst_mac = nodes[dst];
            
            std::cout << "switch rule: id/src/port: " << id << "/" << mac_addr << "/" << src_port << " id/dst: " << dst << "/" << dst_mac << std::endl;
            ...
            //     throw std::runtime_error(dst_mac+std::string(" is an invalid MAC address"));
            // }

            std::cout << std::hex << std::uppercase;
            for (int i = 0; i < 5; i++) {
                std::cout << std::setw(2) << std::setfill('0') << unsigned(bytes[i]) << ":";
            }
            std::cout << std::setw(2) << std::setfill('0') << unsigned(bytes[5]);
            std::cout << std::dec;
...
void apply_gcl(json &data, const int port) {
    ...
    for (auto i = 0; i < time_points.size(); ++i) {
        int time_point = time_points[i];
        std::cout << time_point;

        if (i == time_points.size() - 1) break;
...
void setup_gcl() {
    ...
    for (auto &link: j) {
        ...
        //     std::cout << "[ERROR] Could not find link with id " << link_id << std::endl;
        //     exit(1);
        // }
