...
UScaledNs get_current_timestamp() {
	...
	*((unsigned *)(base_ptr+RTC_CTRL)) = RTC_GET_TIME;
	...
	current_ts.nsec = (uint64_t)current_time.second_l * 1000000000 + current_time.nanosecond;
...
int get_current_local_sync_ts(UScaledNs *local, UScaledNs *sync) {
    ...
    *((unsigned *)(base_ptr + RTC_CTRL)) = RTC_GET_TIME;
    ...
    // This is incomplete, but enough.
    local->nsec = (uint64_t)current_time.second_l * 1000000000 + current_time.nanosecond;
    local->nsec_msb = 0;

    sync->subns = (sync_current_time.frac_nano << 8) & 0xFF00;
    // This is incomplete, but enough.
    sync->nsec = (uint64_t)sync_current_time.second_l * 1000000000 + sync_current_time.nanosecond;
    sync->nsec_msb = 0;
    return XST_SUCCESS;
}
...
int rtc_comp(LocalClockTimestamp t1, LocalClockTimestamp t2) {
    if (t1.second_h > t2.second_h) {
        ...
        return -1;
    } else if (t1.nanosecond > t2.nanosecond) { // second_h and second_l are both equal.
        return 1;
...
void rtc_print_timestamp(LocalClockTimestamp t) {
    printf("%08X %08X sec, %08X ns, %08X frac ns. \r\n", t.second_h, t.second_l, t.nanosecond, t.frac_nano);
}
