...
void sleepns(uint64_t nanoseconds)
{
	...
	ts.tv_nsec = (nanoseconds % 1000000) * 1000000000;
	nanosleep(&ts, NULL);
}
...
int main(int argc,char **argv)
{
	...
	while(run_openplc)
	{
        ...
        // while (1) {
        //     get_current_local_sync_ts(&tmp, &current_ts);
        //     // printf("while(1) timestamp is %" PRIu64 " ns.\n", current_ts.nsec);
        //     if (current_ts.nsec >= next_compute_ts.nsec) {
        //         // output sync time
        //         printf("deterministic send timestamp is %" PRIu64 " ns.\n", current_ts.nsec);
                
        //         // output cpu time
        //         // struct timespec ts;
        //         // clock_gettime(CLOCK_REALTIME, &ts);
        //         // uint64_t cpu_timestamp = ts.tv_sec * 1000000000 + ts.tv_nsec;
        //         // printf("CPU timestamp is %lld.%.9ld ns.\n",  (long long)ts.tv_sec, ts.tv_nsec);
        //         break;
        //     }
        // }
		...
		updateTime();
        // char tmp[100];
        // sprintf(tmp, "common_ticktime is %llu\n", common_ticktime__);
        // log(tmp); 
